...
Le processeur que vous utilisez possède certainement des unités de calcul
vectorielles. Ces unités permettent de faire des opérations sur plusieurs
données en une seule instruction. 

Néanmoins, pour que le compilateur puisse utiliser ces unités, il faut que les
données soient alignées en mémoire. Comme vu en cours, des transformations de
boucles permettent parfois d'éliminer le stride et de rendre les accès mémoire
contigus.

. Modifiez l’ordre des boucles dans votre implémentation de la
multiplication matricielle de manière à ce que la boucle sur `j` soit
la boucle la plus interne. Soit un ordre des boucles `i`, `k`, `j`.

. Mesurez à nouveau la consommation énergétique et le temps d’exécution
pour différentes tailles de matrices. Comparez les résultats avec ceux de
